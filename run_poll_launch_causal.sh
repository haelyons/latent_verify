#!/usr/bin/env bash
# Poll Lambda for ANY single-GPU A100/H100; launch the CAUSAL steer + DLA localize. Single poller.
set -uo pipefail
...
" 2>/dev/null)
  if [ -n "$PICK" ]; then
    ...
    echo "[poll $i] capacity: $TYPE @ $REGION -> launching CAUSAL + DLA localize"
