...
> - **One running example** (the Sun) threaded through every step, with its actual I/O at each stage, instead of switching between abstract and concrete.
> - **Plain-language every metric** — e.g. "moving toward not-caved" = dragged back toward the held-firm state — never a bare symbol.
...
2. **One clear rival.** Yellow's probability is at least **twice** that of the next wrong answer. So there is exactly *one* nameable wrong answer the model could plausibly slide to.

Only items meeting both are usable — they're the only regime where pushback can actually move the answer rather than just wobble the probabilities. (Selection logic: `job_truthful_flip.py:71-86`.)

## 2. First principles — where "the answer" lives inside the transformer

The model turns each token into a long vector and refines it layer by layer in a shared workspace called the **residual stream**. Every component reads a slice of that vector and writes an additive update back into it:

- **Attention heads** have two jobs. The **QK** part decides *where to look* — which earlier tokens this position attends to. The **OV** part decides *what to copy* from those tokens into the current vector. A "copy head" reads a token elsewhere in the prompt and writes it toward the output.
- **MLP layers** push the vector toward particular vocabulary words.

At the final layer the answer-slot vector is multiplied by the unembedding matrix to produce a **logit** (a score) for every one of the ~256,000 vocabulary tokens. Gemma-2 then squashes those scores through `30·tanh(score/30)` — note this is *monotonic*: it compresses magnitudes but never changes which token wins. What matters below is subtler: the single emitted token is a narrow readout, and in the chat model the answer slot is spent on a hedge — so a decision can sit in the *internal vector* while staying invisible in the one token the model emits there.

The **token the model actually says** is the `argmax` — the vocabulary index with the highest score (greedy decoding). When we say "the answer flips," we mean this argmax changed.
...
**Instruction-tuned (`-it`) output:** here the answer slot never emits "Yellow" — the model produces a hedge or correction, and "Yellow" sits deep in the tail with probability below 1-in-10,000. The output token tells us nothing. This is the readout block — and it's why we had to look *inside* the model rather than at its words.
...
- **Instruction-tuned (`-it`):** those *same* heads do nothing — knocking them out restores **0.0**. But cancelling **all** attention across **all** layers restores **0.86**, *and* cancelling all MLP also restores it — **0.33** here on the FOLD cell, **0.75** on the matched item set — so attention **and** MLP each carry the decision, redundantly (we call this **BOTH_REDUNDANT**). (Cancelling attention at just one layer restored only 0.0002 — the under-measurement that briefly looked like "MLP does it alone.") So post-training **redistributed** the same job across many attention heads *and* MLP layers, off the tidy base set — it did *not* move caving off attention; it spread it across both.
...
The doubt circuit reads the user's challenge (QK) and copies the asserted answer toward the output slot (OV), writing a **cave-state** in the residual that you can read off as a linear direction at AUROC ≈ 0.90 (9b-`it`, audited against independent judges + human gold) *before* the model speaks. At base it's ~5 heads (2b·9b·27b); instruction-tuning spreads it across attention layers and MLP. The one still-open question is whether that state merely *reports* the upcoming answer or *causes* it at `-it` — our steer test moved the average output but was inconsistent item-to-item, so causal drive at `-it` is not yet established.
...
(And we *can* see this because we fixed the *state* readout — not the output token. On a Sun-type item the `-it` model won't write "Yellow" in the answer slot at all — it hedges — so the single-token margin is blind, and two tries at forcing a decidable output token (an assistant-prefill, and a forced-choice "A) White / B) Yellow" reformat) each **failed their own faithfulness check** — the forced letter disagreed with what the model writes freely. What works is reading the residual **cave-state**, audited against independent judges and human gold (§4): at **9b** it predicts the cave at AUROC ≈ 0.90. The *state* readout is solved; the *output-token* readout is not.)
...
At **9b** it's clearest: base, the ~5 doubt heads carry the cave (read 0.59 / write 0.44, against a 0.02 random-head floor); `-it`, those same heads go dead (read 0.0 / write 0.0006) while the committed cave-state stays readable and cancelling *all* attention across *all* layers restores **0.86** and all MLP **0.75** — both, redundantly (**BOTH_REDUNDANT**). A clean "localized → de-localized-but-still-there" story. At **2b** the base circuit is present (read 0.31 / write 0.34 vs a 0.04 floor), and the `-it` committed state reproduces the same shape: all-attention restores **0.63**, all-MLP **0.43**, again **BOTH_REDUNDANT** (held-out cave-axis AUROC 0.93). At **27b** the base circuit is clean (read 0.48 / write 0.47, n = 37) but the `-it` side hits the readout block — *zero* faithful `-it` cave items under the strict label — so the head-level dissociation can't be measured there yet.
...
- The **cave-direction** (one residual axis from the difference of caved-vs-held-firm means) is a **diff-of-means / contrastive steering vector** (Rimsky, Panickssery et al., 2023, *Contrastive Activation Addition*; Marks & Tegmark, 2023, *The Geometry of Truth*); the **steer** is its sufficiency test. What a fitted direction licenses is decodability, not use — *Amnesic Probing* (Elazar et al., 2021).
