#!/usr/bin/env bash
# Poll Lambda for ANY single-GPU A100/H100 (fits 9b); launch the residstate CLOSE the moment one is free.
# Broad type list (incl h100_pcie, which the narrow v5 poller missed). Single poller; NO concurrent manual launch.
set -uo pipefail
...
" 2>/dev/null)
  if [ -n "$PICK" ]; then
    ...
    echo "[poll $i] capacity: $TYPE @ $REGION -> launching CLOSE"
    REMOTE_TIMEOUT=9000 bash lambda_run.sh "$TYPE" "$REGION" run_residstate_close.sh results_residstate_close
