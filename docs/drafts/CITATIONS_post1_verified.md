...
**2502.08177** — *SycEval: Evaluating LLM Sycophancy*, Aaron Fanous, Jacob Goldberg, Ank Agarwal, Joanna
Lin, Anson Zhou, Sonnet Xu, Vasiliki Bikia, Roxana Daneshjou, Sanmi Koyejo (all Stanford). Cite as
**Fanous et al. 2025**. *Fetched and re-verified 2026-07-28 against the arXiv v4 HTML (19 Sep 2025) AND
the AAAI camera-ready PDF; the two agree on every number below.* Previously in this ledger for the
progressive/regressive vocabulary only — the asymmetry is now verified too.
- **DOI RESOLVES.** `https://doi.org/10.1609/aies.v8i1.36598` returns HTTP 302 →
  `https://ojs.aaai.org/index.php/AIES/article/view/36598` (HTTP 200). It is SycEval. Venue:
  *Proceedings of the AAAI/ACM Conference on AI, Ethics, and Society*, **Vol. 8, No. 1, pp. 893–900,
  2025**, published 15 Oct 2025, ISSN 3065-8365. So the researcher's live L20 link is correct, and
  `HOLES_post1_v2.md` §4's "Not in CIT." is discharged.
- **THE ASYMMETRY IS IN THE PAPER, AS AN EXPLICIT NUMBER.** Results, §"Sycophancy Rates Are High Across
  Models" (also in the abstract and in Discussion §"Summary of Findings"): "Our experiments showed that
  58.19% of all samples exhibited sycophantic behavior, with progressive responses and regressive
  responses occurring at **43.52% and 14.66%**, respectively." Progressive = toward the correct answer
  = this post's *listen*; regressive = toward the incorrect = *fold*.
- **It holds for all three models**, same section: Gemini 53.22% prog / 9.25% regr; ChatGPT 42.32% /
  14.40%; Claude-Sonnet 39.13% / 18.31%. No model has regressive above progressive in aggregate.
- **It does NOT hold uniformly per dataset**, and this must be carried whenever the number is used.
  Table 2 (AMPS Math) prog/regr: ChatGPT 899/38 in-context, 1029/124 preemptive; Gemini 767/46, 647/104;
  Claude 746/77, 765/142. Table 3 (MEDQuad): ChatGPT 469/393, 457/416 (nearly even); Gemini 138/82,
  185/70; **Claude 302/383 and 275/375 — regressive exceeds progressive**. The headline gap is carried
  by the maths half; on medical advice it closes for ChatGPT-4o and reverses for Claude-Sonnet.
- **Models: ChatGPT-4o-(2024-05-13), Claude-Sonnet, Gemini-1.5-Pro**, "under default calibration
  settings" (Methods, Step 1). All three are deployed chat assistants. **There is no base / pretrained
  checkpoint anywhere in the paper**, so SycEval supports "chat models show this asymmetry" and supports
  **nothing** about base-vs-chat. It is also not Gemma and not open-weight. Datasets: 500 AMPS Math +
  500 MedQuad items, 3000 initial + 24000 rebuttal queries, **15345 non-erroneous responses** analysed;
  LLM-as-a-judge is ChatGPT-4o-(2024-08-06) at temperature 0, calibrated against 20 human labels per
  dataset (one maths undergraduate, one MD).
- **CONFOUND, and it is real.** The two directions are run on disjoint item pools chosen by the model's
  own initial correctness (Methods, Step 2): "If the initial inquiry response was correct, we present
  evidence justifying an incorrect answer … If the initial inquiry response was incorrect, we present
  evidence justifying the correct answer." Both counts are then divided by the **same** 15345 total —
  43.52% + 14.66% = 58.19% reproduces exactly from Tables 2 and 3 (prog 6679, regr 2250) — so they share
  a denominator but not an opportunity set, and **the initial correct/incorrect split is never
  reported**, so a per-opportunity rate cannot be recovered from the published text. On a maths set the
  models start wrong far more often than not, which is most of why AMPS shows 4853 progressive against
  531 regressive. "Revise toward truth more readily" is therefore fair as a description of *what they
  report*, and is NOT a like-for-like propensity comparison. This post's design does not share the
  problem: the same 82 pairs go through both directions.
- Second confound, smaller: the aggregate mixes two rebuttal formats whose regressive rates differ —
  "Preemptive responses exhibit significantly higher regressive sycophancy rates than in-context
  responses across datasets (P<0.001)", AMPS preemptive 8.13% vs in-context 3.54%, while "progressive
  sycophancy rates are similar between preemptive and in-context responses across both datasets". So
  the regressive half of 43.52/14.66 is format-sensitive in a way the progressive half is not.
- Other citable lines, if ever needed: rebuttals are generated by llama3 8b, four strengths nested
  simple ⊆ ethos ⊆ justification ⊆ citation-and-abstract; "simple rebuttals were effective in maximizing
  progressive sycophancy (Z=6.59, p<0.001) while citation rebuttals produced the most regressive"; overall
  persistence 78.5% (95% CI [77.2%, 79.8%]).
- Their own framing of the dichotomy, Related Works: "Existing discussions of sycophancy focus on what
  we term regressive sycophancy—when a model conforms to an incorrect user belief. However, in many real
  interactions, the opposite also occurs: progressive sycophancy, when a user provides a correct
  statement and agreement is in fact desirable."
- **What the post may now claim**: that SycEval reports progressive sycophancy at 43.52% against
  regressive at 14.66%, that this ordering holds for each of their three chat models, that their models
  are all chat with no base arm, and that the gap is carried by their maths set and reverses for
  Claude-Sonnet on medical advice. What it may **not** claim: that SycEval says anything about base
  models, that the two rates are comparable propensities, or that the asymmetry is uniform.
