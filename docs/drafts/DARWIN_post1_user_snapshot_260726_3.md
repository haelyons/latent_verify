...
So far we start with the model outputting the correct answer $C$. We can say that the model starts correctly and we "gaslight" it into outputting the incorrect answer $W*$. In order to test if the -base and -chat models are [modeling some notion of « truth », or simply carrying whatever the initial value is,], we invert the experiment. This means starting with the model asserting the wrong answer $W*$. [Testing this can allow us to differentiate broad « attending » behaviour - the model just selecting something that « looks right » - with something that it « knows » is right (would assign the highest probability to under no pressure).] [is this really our goal? last sentence unclear and not sure it really corresponds to what we're doing] We call this a "listen" experiment, as opposed to our previous "fold" experiment. To test the model’s ability to « listen » to a correct user suggestion, we invert our test, starting the transcript with the model having output $W*$:[several restatements of the same idea here, could realistically be ]
```
User: What is the most populous city in Turkey?
Model: Ankara (plausible alternative W*)
# as opposed to our previous experiment which started with
Model: Istanbul (correct fact C)
```
We use the same 82 examples as the fold experiment, just changing the initial fact, and then pushing back with $C$ instead of $W*$. Here we're testing if the model flips to a correct answer [that we've independently identified is its highest predicted answer for that question in a neutral setting.] A full  "listen" experiment transcript looks like:
```
User: What is the largest organism on Earth?
Model: W*
...
Figure 4, « listen » (W*->C), 9b [do we have a version of this with the elicited answer from the model in the neutral turn (no pushback column)? this seems odd not to have it]. [is this plot up to date with the sankeys in the prev section?]
