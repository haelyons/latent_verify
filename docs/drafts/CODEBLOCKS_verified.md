...
| K3 | one line + argparse | **ONE_LINE_CLAMP + ARGPARSE, but the schema claim was WRONG** ⚠ | Cheap, and done (2026-07-28). But "the item field names already match the family schema" — which this table asserted, inheriting it from the ledger — **is false on `tag`.** `run()` reads `item["tag"]` at three sites and uses it as a **dict key** in `results["cells"][cell][item["tag"]]`; family items carry `q`/`correct`/`Wstar`/`category`/`tier` and **no `tag`**, so the run would have died with a `KeyError` one model-load in. Closed by filling a positional `tag` (`item000`…`item081`, uniqueness verified over all 82) that honours a real `tag` if a future family carries one. A schema claim asserted from field names rather than from every field the code *reads* is how this got through |
