...
reply and then the forced final answer (planted, reply, elicited). One string-identity register
throughout, the same one used for the body figures: a segment is coloured only if the turn spells the
answer out.
...
answer in it (a hedge string), at -chat mostly not that at all. A turn is BOTH when the matcher
returns NEITHER/UNRESOLVED\_ALIAS *and* the isolated answer span names both the correct and the W\*
entity under the labeller's own word-boundary entity forms. The full rationale — including why the
base and -chat gray bands were previously incomparable, and the plural-form matcher limitation that
leaves a small residue in gray — is in `figB_neutral_counterfactual_caption.md`; it applies verbatim
here.

In this cell the split matters most on the -chat pushback reply: of the 14 replies the matcher cannot
resolve, **13 name both answers** and only 1 names neither. Read as a single gray band, that column
looked like withholding; it is almost entirely ambivalence.
