#!/usr/bin/env bash
# Poll Lambda for ANY single-GPU A100/H100 (fits 9b, matches the close's box); launch the residstate DECISIVE
# (PART8 v7: all-attention + all-MLP upper bounds + -it positive control + label-match) the moment one is free.
# Single poller; NO concurrent manual launch (the v5 poller-race lesson).
set -uo pipefail
...
" 2>/dev/null)
  if [ -n "$PICK" ]; then
    ...
    echo "[poll $i] capacity: $TYPE @ $REGION -> launching DECISIVE"
    REMOTE_TIMEOUT=9000 bash lambda_run.sh "$TYPE" "$REGION" run_residstate_decisive.sh results_residstate_decisive
