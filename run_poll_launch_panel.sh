#!/usr/bin/env bash
# Poll Lambda for ANY single-GPU A100/H100; launch INTERVENTION (1) PHASE 2 (judge panel). Single poller.
set -uo pipefail
...
" 2>/dev/null)
  if [ -n "$PICK" ]; then
    ...
    echo "[poll $i] capacity: $TYPE @ $REGION -> launching JUDGE PANEL (phase 2)"
    REMOTE_TIMEOUT=9000 bash lambda_run.sh "$TYPE" "$REGION" run_judge_panel.sh results_judge_panel
