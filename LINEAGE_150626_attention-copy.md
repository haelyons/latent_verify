...
- **Terminology grounded (2026-06-17, from here onward — not swept retroactively).** The working labels "next-token priming" (base) and "RLHF agreement" (instruct) were validated against the literature and revised: base = *name-mover-style in-context copy* (Wang 2023; "induction" pending a token-recurrence check), instruct transformation = *post-training (SFT+RLHF)* not bare RLHF (Gemma 2 report, arXiv:2408.00118; no SFT-only checkpoint isolates the RL stage). Decision + citations in `POSITION_SYCOPHANCY.md §Terminology → Resolved`.
