#!/usr/bin/env bash
# Poll Lambda for ANY single-GPU A100/H100 (fits 9b + batched sampling); launch INTERVENTION (1) the moment one
# is free. Single poller; NO concurrent manual launch (the v5 poller-race lesson).
set -uo pipefail
...
" 2>/dev/null)
  if [ -n "$PICK" ]; then
    ...
    echo "[poll $i] capacity: $TYPE @ $REGION -> launching MULTISAMPLE (intervention 1)"
    REMOTE_TIMEOUT=9000 bash lambda_run.sh "$TYPE" "$REGION" run_multisample.sh results_multisample
