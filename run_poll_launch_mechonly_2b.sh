#!/usr/bin/env bash
# Poll Lambda for a cheap box (A10 24GB fits 2b; fall back A100) and launch the MECHONLY 2b run.
set -uo pipefail
...
" 2>/dev/null)
  if [ -n "$PICK" ]; then
    ...
    echo "[poll $i] capacity: $TYPE @ $REGION -> launching MECHONLY 2b"
    REMOTE_TIMEOUT=5400 bash lambda_run.sh "$TYPE" "$REGION" run_mechonly_2b.sh results_mechonly_2b
