#!/usr/bin/env bash
# On-box: PROMPT FEATURE -> caving MECHANISM map (2b, copy's home). Same model + faithful readout; vary only the
...
echo "=== 2b prompt-feature -> mechanism, big pool (base qa + it qa) ==="
python cave_prompt_feature_mechanism.py --device cuda --big-pool \
  --name-base google/gemma-2-2b --name-it google/gemma-2-2b-it --tag 2b > out/cave_prompt_feature_mechanism_2b.log 2>&1
echo "exit=$?"
echo "--- tail ---"; tail -60 out/cave_prompt_feature_mechanism_2b.log
echo "ALLDONE_PROMPTFEAT_2B"
