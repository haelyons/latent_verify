...
- **`docs/lambda-gpu-access.md`** — the run path: GPU runs are driven over SSH from
  the workstation (not the web sandbox), via the Lambda Cloud API; $500 cap + a spend
  tally; terminate when done.
- **`latent_skeptic/`** — the adversarial triage harness that gates every load-bearing
  claim (H1: skeptics share no state; H2: a crux is verified by running, not reading).
