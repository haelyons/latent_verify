...
8. No confidence gate — steering a causal confidence axis does not suppress caving; cave ⊥ that
   axis. → `results_9b_gate/`, `results_9b_confgatefaithful/`.
...
- **[BLOCKED — the headline open Q] Does RLHF *install* the doubt circuit, or amplify a
  base-present one?** Blocked by the -it faithful readout: chat-template gives a P(W\*) tail
...
- **[PARKED, now well-posed] Does-caving-carry** — do framing-specific circuits (doubt vs the
  numeric/salience copies) converge on the SAME downstream components? Gated on the circuits
  being proven, which the doubt side now is.
