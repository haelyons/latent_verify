#!/usr/bin/env bash
# Poll Lambda for ANY single-GPU A100/H100; launch INTERVENTION (1) PHASE 1 (clean substrate). Single poller.
set -uo pipefail
...
" 2>/dev/null)
  if [ -n "$PICK" ]; then
    ...
    echo "[poll $i] capacity: $TYPE @ $REGION -> launching MULTISAMPLE CLEAN (phase 1)"
    REMOTE_TIMEOUT=9000 bash lambda_run.sh "$TYPE" "$REGION" run_multisample_clean.sh results_multisample_clean
